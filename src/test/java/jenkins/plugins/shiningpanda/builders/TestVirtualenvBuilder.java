...
public class TestVirtualenvBuilder extends ShiningPandaTestCase {
	...
	public void testRoundTripFreeStyle() throws Exception {
		...
		VirtualenvBuilder before = new VirtualenvBuilder(
				installation.getName(), "env2", true, false,
				CommandNature.SHELL.getKey(), "echo hello", true);
		VirtualenvBuilder after = configFreeStyleRoundtrip(before);
		assertEqualBeans2(before, after,
				"pythonName,home,clear,systemSitePackages,nature,command,ignoreExitCode");
	}

	public void testRoundTripMatrix() throws Exception {
		VirtualenvBuilder before = new VirtualenvBuilder("foobar", "env2",
				true, false, CommandNature.PYTHON.getKey(), "echo hello", false);
		VirtualenvBuilder after = configPythonMatrixRoundtrip(before);
		assertEqualBeans2(before, after,
				"home,clear,systemSitePackages,nature,command,ignoreExitCode");
	}
	...
	public void testStandardPythonHomeWithSpace() throws Exception {
		...
		VirtualenvBuilder builder = new VirtualenvBuilder(
				installation.getName(), "env", false, false,
	...
	public void testTextAxisAvailable() throws Exception {
		...
		VirtualenvBuilder builder = new VirtualenvBuilder(null, "env", true,
				false, CommandNature.SHELL.getKey(), "echo \"Welcome $TOTO\"",
				false);
		MatrixProject project = createMatrixProject();
		AxisList axes = new AxisList(new PythonAxis(
				new String[] { installation.getName(), }), new TextAxis("TOTO",
				"TUTU"));
	...
	public void testInvalidPythonName() throws Exception {
		configureCPython2();
		String name = "Toto";
		VirtualenvBuilder builder = new VirtualenvBuilder(name, "env", true,
				false, CommandNature.SHELL.getKey(), "echo \"Welcome $TOTO\"",
				false);
		FreeStyleProject project = createFreeStyleProject();
		project.getBuildersList().add(builder);
	...
	public void testNoPythonAxis() throws Exception {
		...
		assertTrue("a missing python axis was expected:\n" + log,
				log.contains(Messages.BuilderUtil_PythonAxis_Required()));
	}
	...
	public void testNoVirtualenvClear() throws Exception {
		...
		assertTrue("a new virtualenv was expected to be created:\n" + log,
				log.contains("New python executable in"));
