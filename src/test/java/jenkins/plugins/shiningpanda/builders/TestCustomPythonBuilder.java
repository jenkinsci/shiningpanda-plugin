...
public class TestCustomPythonBuilder extends ShiningPandaTestCase {
    ...
    public void testRoundTripFreeStyle() throws Exception {
        CustomPythonBuilder before = new CustomPythonBuilder("/tmp/custom", CommandNature.SHELL.getKey(), "echo hello",
                true);
    ...
    public void testRoundTripMatrix() throws Exception {
        CustomPythonBuilder before = new CustomPythonBuilder("/tmp/custom", CommandNature.PYTHON.getKey(), "echo hello",
                false);
    ...
    public void testHomeWithSpace() throws Exception {
        CustomPythonBuilder builder = new CustomPythonBuilder(
                createFakePythonInstallationWithWhitespaces().getAbsolutePath(), CommandNature.SHELL.getKey(),
                "echo hello", false);
    ...
    public void testPythonNature() throws Exception {
        CustomPythonBuilder builder = new CustomPythonBuilder(getCPython3Home(), CommandNature.PYTHON.getKey(),
    ...
    public void testXShellNature() throws Exception {
        CustomPythonBuilder builder = new CustomPythonBuilder(getCPython3Home(), CommandNature.XSHELL.getKey(),
                "echo %HOME%", false);
        FreeStyleProject project = j.createFreeStyleProject();
        project.getBuildersList().add(builder);
        FreeStyleBuild build = project.scheduleBuild2(0).get();
