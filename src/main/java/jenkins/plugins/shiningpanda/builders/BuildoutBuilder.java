...
public class BuildoutBuilder extends Builder implements Serializable {
	...
	public BuildoutBuilder(String pythonName, String buildoutCfg,
	...
	public boolean perform(AbstractBuild<?, ?> build, Launcher launcher,
		...
		PythonInstallation installation = BuilderUtil.getInstallation(build,
				listener, environment, pythonName);
		// Check if an installation was found
		if (installation == null)
			// If not installation found, do not continue the build
			return false;
		// Get the PYTHON
		Python interpreter = BuilderUtil.getInterpreter(launcher, listener,
		...
		Virtualenv virtualenv = BuilderUtil.getVirtualenv(listener,
				workspace.getBuildoutHome(installation.getName()));
		...
		if (virtualenv.isOutdated(workspace, interpreter, vSystemSitePackages))
			...
			if (!virtualenv.create(launcher, listener, workspace, pwd,
					environment, interpreter, vSystemSitePackages))
				// Failed to create the environment, do not continue
				return false;
		// Bootstrap
		if (!virtualenv.buildout(launcher, listener, workspace, pwd,
				environment, buildoutCfg))
		...
		environment.override("PATH+",
				pwd.child(buildoutCfg).getParent().child("bin").getRemote());
		// Launch script
		return BuilderUtil.launch(launcher, listener, pwd, environment,
				virtualenv, nature, command, ignoreExitCode);
	}
	...
	public static final class DescriptorImpl extends
		...
		public String getHelpFile() {
			return Functions.getResourcePath()
					+ "/plugin/shiningpanda/help/builders/BuildoutBuilder/help.html";
		}
