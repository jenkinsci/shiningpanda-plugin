	...
	public boolean perform(AbstractBuild<?, ?> build, Launcher launcher,
		...
		if (!environment.containsKey(ToxAxis.KEY)) {
			...
			if (toxenvPattern != null) {
				...
				String toxenv = Util.fixEmptyAndTrim(Util.replaceMacro(
						toxenvPattern, new UnixVariableResolver(environment)));
				...
				// Invalid TOXENV value
					...
					listener.fatalError(Messages
							.ToxBuilder_ToxenvPattern_Invalid(toxenvPattern));
		...
		if (!virtualenv.pipInstall(launcher, listener, workspace, pwd,
				environment, "tox"))
		...
		if (FilePathUtil.isUnix(pwd)) {
			...
			List<Python> contributors = BuilderUtil.getInterpreters(launcher,
		...
		// Launch TOX
		return virtualenv.tox(launcher, listener, pwd, environment, toxIni,
				recreate);
	}
	...
	public static final class DescriptorImpl extends
		...
		public String getHelpFile() {
			return Functions.getResourcePath()
					+ "/plugin/shiningpanda/help/builders/ToxBuilder/help.html";
		}
		...
		public boolean isApplicable(
			...
			return !PythonInstallation.isEmpty()
					&& jobType.equals(MatrixProject.class);
		}
