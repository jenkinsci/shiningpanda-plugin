...
public class Virtualenv extends Python {
	...
	public boolean isValid() throws IOException, InterruptedException {
		...
		if (isWindows()) {
			...
			if (FilePathUtil.existsOrNull(
					getHome().child("bin").child("activate.bat"), getHome()
							.child("Scripts").child("activate.bat")) == null)
				// Not found, this VIRTUALENV is not valid
				return false;
		}
		// If on UNIX, look for activate script
		else if (FilePathUtil.existsOrNull(getHome().child("bin").child(
				"activate")) == null)
			// Not found, this VIRTUALENV is not valid
			return false;
	...
	public Map<String, String> getEnvironment(boolean includeHomeKey)
		...
		if (isWindows()) {
			...
			// In a scripts one
				...
				environment
						.put("PATH+", getHome().child("Scripts").getRemote());
	...
	public FilePath getExecutable() throws IOException, InterruptedException {
		...
		if (isWindows())
			...
			return FilePathUtil.existsOrNull(
					getHome().child("bin").child("jython.bat"), getHome()
							.child("bin").child("pypy.exe"),
					getHome().child("Scripts").child("python.exe"));
		// On UNIX look for executable in bin folder
		return FilePathUtil.existsOrNull(
				getHome().child("bin").child("jython"), getHome().child("bin")
						.child("pypy"), getHome().child("bin").child("python"));
	}
	 ...
	 * @throws InterruptedException
	 */
	public boolean isOutdated(Workspace workspace, Python interpreter,
			boolean systemSitePackages) throws IOException,
			InterruptedException {
		// Out dated if invalid, or if no signature file, or if signatures
		// differ
		return !isValid()
				|| !getSignatureFile().exists()
				|| !FilePathUtil.read(getSignatureFile(), "UTF-8")
						.equals(getSignature(workspace, interpreter,
								systemSitePackages));
	}
	 ...
	 * @throws IOException
	 */
	public boolean create(Launcher launcher, TaskListener listener,
			Workspace workspace, FilePath pwd, EnvVars environment,
			Python interpreter, boolean systemSitePackages)
		...
		// Start creation
		boolean success = LauncherUtil.launch(
				launcher,
				listener,
				pwd,
				EnvVarsUtil.override(environment,
						interpreter.getEnvironment(includeHomeKey)), args);
		...
		if (isUnix()) {
			...
			if (!libs.isEmpty()) {
				...
				for (FilePath lib : libs) {
					...
					if (!link.exists())
						// Create the link
						if (!LauncherUtil.createSymlink(launcher, listener,
								lib, link))
		...
		if (success)
			...
			getSignatureFile().write(
					getSignature(workspace, interpreter, systemSitePackages),
					"UTF-8");
		// Return success flag
		return success;
	}
	 ...
	 * @throws IOException
	 */
	public boolean pipInstall(Launcher launcher, TaskListener listener,
			Workspace workspace, FilePath pwd, EnvVars environment,
		...
		if (extraSearchDir != null)
			// Add flag
			args.add("-f")
					.add(workspace.getPackagesDir().toURI().toURL()
							.toExternalForm());
		// Ask for upgrade
		args.add("--upgrade");
		// The package to install
		...
		return LauncherUtil.launch(launcher, listener, pwd,
	 ...
	 * @throws IOException
	 */
	public boolean tox(Launcher launcher, TaskListener listener, FilePath pwd,
			EnvVars environment, String toxIni, boolean recreate)
			throws InterruptedException, IOException {
		// Create the arguments for the command line
		ArgumentListBuilder args = new ArgumentListBuilder();
	 ...
	 * @throws IOException
	 */
	public boolean buildout(Launcher launcher, TaskListener listener,
			Workspace workspace, FilePath pwd, EnvVars environment,
			String buildoutCfg) throws InterruptedException, IOException {
		// Get the environment
		EnvVars finalEnvironment = EnvVarsUtil.override(environment,
				getEnvironment());
		// Create the arguments for the command line
		ArgumentListBuilder args = new ArgumentListBuilder();
		// Add the path to PYTHON executable
		...
		// Start bootstrap
		if (!LauncherUtil.launch(launcher, listener, pwd, finalEnvironment,
				args))
			// Failed to bootstrap, no need to go further
			return false;
		// Call BUILDOUT and return status
		return LauncherUtil.launch(launcher, listener, pwd, finalEnvironment,
				new ArgumentListBuilder(pwd.child(buildoutCfg).getParent()
						.child("bin").child("buildout").getRemote(), "-c",
						buildoutCfg));
	}
	 ...
	 * @throws InterruptedException
	 */
	public static String getSignature(Workspace workspace, Python interpreter,
			boolean systemSitePackages) throws IOException,
			InterruptedException {
