...
public abstract class Workspace {
	...
	public FilePath getVirtualenvPy() throws IOException, InterruptedException {
		// TODO: optimize transfer of PIP and SETUPTOOLS
		getHome().child(SETUPTOOLS)
				.copyFrom(getClass().getResource(SETUPTOOLS));
		getHome().child(PIP).copyFrom(getClass().getResource(PIP));
		return FilePathUtil.synchronize(getHome().child(VIRTUALENV),
	...
	public FilePath getBootstrapPy() throws IOException, InterruptedException {
		return FilePathUtil.synchronize(getHome().child(BOOTSTRAP),
	 ...
	 * @throws InterruptedException
	 */
	public FilePath getMasterPackagesDir() throws IOException,
			InterruptedException {
		return FilePathUtil.isDirectoryOrNull(Jenkins.getInstance()
				.getRootPath().child(BASENAME).child(PACKAGES));
	}
	 ...
	 * @throws InterruptedException
	 */
	public abstract FilePath getPackagesDir() throws IOException,
			InterruptedException;
	...
	public FilePath getVirtualenvHome(String name) {
		return getHome().child("virtualenvs").child(
				Util.getDigestOf(Util.fixNull(name)).substring(0, 8));
	}
	...
	public FilePath getBuildoutHome(String name) {
		return getHome().child("buildouts").child(
				Util.getDigestOf(Util.fixNull(name)).substring(0, 8));
	}
	...
	protected void delete() {
		...
		try {
			...
			// Log
			LOGGER.log(Level.SEVERE, "Failed to delete workspace: "
					+ getHome().getRemote(), e);
		}
	}
	...
	public static Workspace fromHome(FilePath home) {
		return home.isRemote() ? new SlaveWorkspace(home)
				: new MasterWorkspace(home);
	}
	...
	public static Workspace fromNode(Node node, AbstractProject<?, ?> project,
		...
		if (project instanceof MatrixConfiguration)
			...
			id = (name != null ? name : ((MatrixConfiguration) project)
					.getParent().getName()) + project.getName();
		...
		return fromHome(WorkspaceHomeProperty.get(node).child(
				Util.getDigestOf(id).substring(0, 8)));
	}
	...
	public static void delete(Item item, String name) {
		...
		if (item instanceof MatrixProject)
			...
			for (MatrixConfiguration configuration : ((MatrixProject) item)
					.getItems()) {
				// Get workspace
